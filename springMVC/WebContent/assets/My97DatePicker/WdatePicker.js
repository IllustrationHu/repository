...
function K(A,$,B){
	...
	if(D){_.href=A;_.rel="stylesheet";
	_.type="text/css";
	if($)_.title=$;
	if(B)_.charset=B;
	D.appendChild(_)}}
	...
	if(G.getBoundingClientRect)
		...
		if(C!=G)
			while(C){
				...
				else if(C.tagName.toLowerCase()=="body")I=C.ownerDocument.defaultView;C=C.offsetParent}C=G.parentNode;
...
function b($){
	$=$||V;
	var B=$[N],A=B[H],_=B.body;B=(A&&A.scrollTop!=null&&(A.scrollTop>_.scrollTop||A.scrollLeft>_.scrollLeft))?A:_;
...
function Z(){
	$dp.status=2}var Q,_;
	function U(K,C){if(!$dp)return;
	B();
	var L={};for(var H in K)L[H]=K[H];
	for(H in $)if(H.substring(0,1)!="$"&&L[H]===undefined)L[H]=$[H];
	if(C){if(!J()){_=_||setInterval(function(){if(V[N].readyState=="complete")clearInterval(_);
	U(null,true)},50);
	return}
	if($dp.status==0){$dp.status=1;L.el=T;I(L,true)}
		...
		I(L)}else{
			...
			F.cancelBubble=true}L.el=L.el=$dp.$(L.el||L.srcEl);
	...
	function D(){
		if(G){
			func=D.caller;
			while(func!=null){var $=func.arguments[0];
	...
	function I(G,_){
		...
		if(!$dp.dd||G.eCont||($dp.dd&&(G.getRealLang().name!=$dp.dd.lang||G.skin!=$dp.dd.skin))){
			...
			V[N].body.appendChild($dp.dd);E($dp.dd,G);
		...
		function E(K,J){
			...
			try{H=K.lastChild.contentWindow[N]}
			catch(E){F=true;K.removeChild(K.lastChild);
			...
			function C(){
				...
				H.close()}}
		function B(J){
			var H=J.position.left,C=J.position.top,D=J.el;if(D==T)return;
